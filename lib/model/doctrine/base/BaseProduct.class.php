<?php

/**
 * BaseProduct
 * 
 * This class has been auto-generated by the Doctrine ORM Framework
 * 
 * @property integer $product_category_id
 * @property integer $product_line_id
 * @property string $name
 * @property string $description_short
 * @property string $description_long
 * @property string $price
 * @property string $image
 * @property ProductCategory $ProductCategory
 * @property ProductLine $ProductLine
 * 
 * @method integer         getProductCategoryId()   Returns the current record's "product_category_id" value
 * @method integer         getProductLineId()       Returns the current record's "product_line_id" value
 * @method string          getName()                Returns the current record's "name" value
 * @method string          getDescriptionShort()    Returns the current record's "description_short" value
 * @method string          getDescriptionLong()     Returns the current record's "description_long" value
 * @method string          getPrice()               Returns the current record's "price" value
 * @method string          getImage()               Returns the current record's "image" value
 * @method ProductCategory getProductCategory()     Returns the current record's "ProductCategory" value
 * @method ProductLine     getProductLine()         Returns the current record's "ProductLine" value
 * @method Product         setProductCategoryId()   Sets the current record's "product_category_id" value
 * @method Product         setProductLineId()       Sets the current record's "product_line_id" value
 * @method Product         setName()                Sets the current record's "name" value
 * @method Product         setDescriptionShort()    Sets the current record's "description_short" value
 * @method Product         setDescriptionLong()     Sets the current record's "description_long" value
 * @method Product         setPrice()               Sets the current record's "price" value
 * @method Product         setImage()               Sets the current record's "image" value
 * @method Product         setProductCategory()     Sets the current record's "ProductCategory" value
 * @method Product         setProductLine()         Sets the current record's "ProductLine" value
 * 
 * @package    simple-site
 * @subpackage model
 * @author     Your name here
 * @version    SVN: $Id: Builder.php 7490 2010-03-29 19:53:27Z jwage $
 */
abstract class BaseProduct extends sfDoctrineRecord
{
    public function setTableDefinition()
    {
        $this->setTableName('product');
        $this->hasColumn('product_category_id', 'integer', null, array(
             'type' => 'integer',
             ));
        $this->hasColumn('product_line_id', 'integer', null, array(
             'type' => 'integer',
             ));
        $this->hasColumn('name', 'string', 255, array(
             'type' => 'string',
             'notnull' => true,
             'length' => 255,
             ));
        $this->hasColumn('description_short', 'string', 500, array(
             'type' => 'string',
             'notnull' => true,
             'length' => 500,
             ));
        $this->hasColumn('description_long', 'string', 1000, array(
             'type' => 'string',
             'notnull' => true,
             'length' => 1000,
             ));
        $this->hasColumn('price', 'string', 255, array(
             'type' => 'string',
             'length' => 255,
             ));
        $this->hasColumn('image', 'string', 255, array(
             'type' => 'string',
             'length' => 255,
             ));
    }

    public function setUp()
    {
        parent::setUp();
        $this->hasOne('ProductCategory', array(
             'local' => 'product_category_id',
             'foreign' => 'id'));

        $this->hasOne('ProductLine', array(
             'local' => 'product_line_id',
             'foreign' => 'id'));
    }
}